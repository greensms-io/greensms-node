  ...
  addInterceptors() {
    ...
    this.service.interceptors.request.use(config => {
      if (currentInstance.token) {
        config.headers.Authorization = `Bearer ${currentInstance.token}`;
      }

      if (currentInstance.defaultParams) {
        config.params = { ...config.params,
          ...currentInstance.defaultParams,
        };
      }

      if (currentInstance.defaultData) {
        config.data = { ...config.data,
          ...currentInstance.defaultData,
        };
      }
    ...
    this.service.interceptors.response.use(response => {
      if (currentInstance.useCamelCase) {
