'use strict';var axios=require('axios'),humps=require('humps'),qs=require('qs'),Yup=require('yup');function _interopNamespaceDefault(e){var n=Object.create(null);if(e){Object.keys(e).forEach(function(k){if(k!=='default'){var d=Object.getOwnPropertyDescriptor(e,k);Object.defineProperty(n,k,d.get?d:{enumerable:true,get:function(){return e[k]}});}})}n.default=e;return Object.freeze(n)}var Yup__namespace=/*#__PURE__*/_interopNamespaceDefault(Yup);var __defProp$n = Object.defineProperty;
...
let V1_0$9=class V1_0 {
  constructor(client) {
    ...
    __publicField$k(this, "schema", {
      token: Yup__namespace.object().shape({
        expire: Yup__namespace.number().positive().integer()
      })
    });
  }
  balance() {
...
let V1_0$8=class V1_0 {
  constructor(client) {
    ...
    __publicField$i(this, "schema", {
      receive: Yup__namespace.object().shape({
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        tollFree: Yup__namespace.boolean()
      }),
      send: Yup__namespace.object().shape({
        language: Yup__namespace.string().oneOf(["ru", "en"]),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        voice: Yup__namespace.boolean()
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().length(36)
      })
    });
  }
  receive(params) {
...
let V1_0$6=class V1_0 {
  constructor(client) {
    ...
    __publicField$e(this, "schema", {
      send: Yup__namespace.object().shape({
        amount: Yup__namespace.number().required().min(1).positive(),
        card: Yup__namespace.string().min(11).max(14),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number")
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().min(1).max(36).matches(NUMBER_OR_UUID_REGEXP, "Invalid Status")
      })
    });
  }
  send(params) {
...
let V1_0$5=class V1_0 {
  constructor(client) {
    ...
    __publicField$c(this, "schema", {
      send: Yup__namespace.object().shape({
        from: Yup__namespace.string().min(1).max(11),
        hidden: Yup__namespace.string().min(1).max(918),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        txt: Yup__namespace.string().required().min(1)
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().length(36)
...
let V1_0$4=class V1_0 {
  constructor(client) {
    ...
    __publicField$a(this, "schema", {
      send: Yup__namespace.object().shape({
        cascade: Yup__namespace.string().oneOf(["sms", "voice"]),
        from: Yup__namespace.string().min(1),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        txt: Yup__namespace.string().required().min(1)
      }),
      status: Yup__namespace.object().shape({
...
let V1_0$3=class V1_0 {
  constructor(client) {
    ...
    __publicField$8(this, "schema", {
      send: Yup__namespace.object().shape({
        cascade: Yup__namespace.array().transform(function(value, originalValue) {
          if (this.isType(value) && value !== null) {
            return value;
          }
          return originalValue ? originalValue.split(/[\s,]+/) : [];
        }).of(Yup__namespace.string().oneOf(["sms", "voice", "viber"])),
        from: Yup__namespace.string().min(1).max(11),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        txt: Yup__namespace.string().required().min(1).max(2048)
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().length(36)
...
let V1_0$2=class V1_0 {
  constructor(client) {
    ...
    __publicField$6(this, "schema", {
      send: Yup__namespace.object().shape({
        lang: Yup__namespace.string().oneOf(["ru", "en"]),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        txt: Yup__namespace.string().required().min(1).max(5).matches(/^\d+$/, "Invalid Code")
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().length(36)
...
let V1_0$1=class V1_0 {
  constructor(client) {
    ...
    __publicField$4(this, "schema", {
      send: Yup__namespace.object().shape({
        file: Yup__namespace.string().min(1).max(256),
        from: Yup__namespace.string().min(1),
        tag: Yup__namespace.string().min(1).max(36),
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number"),
        txt: Yup__namespace.string().required().min(1).max(1e4)
      }),
      status: Yup__namespace.object().shape({
        extended: Yup__namespace.boolean(),
        id: Yup__namespace.string().required().length(36)
      }),
      webhook: Yup__namespace.object().shape({
        url: Yup__namespace.string().required().min(1).matches(URL_REGEXP, "Invalid URL")
      })
    });
  }
  send(params) {
...
class V1_0 {
  constructor(client) {
    ...
    __publicField$2(this, "schema", {
      lookup: Yup__namespace.object().shape({
        to: Yup__namespace.string().required().min(11).max(14).matches(/^\d+$/, "Invalid Phone Number")
      })
    });
  }
